...
from visualization_utils import auto_visualize, show_data_summary, parse_text_to_dataframe
...
# UI - Título
st.markdown("""
    <div style="text-align: center; background-color: #333; color: white; padding: 10px; 
                border-radius: 5px; font-size: 24px; font-weight: bold;">
        CONSULTA TU BASE DE DATOS
    </div>
    """, unsafe_allow_html=True)

# UI - Selección de Base de Datos
db_type = st.selectbox("Selecciona el tipo de base de datos:", 
                         ["SQL Server", "PostgreSQL", "MySQL", "SQLite"])

# UI - Inputs de Conexión Dinámicos
conn_args = {}
if db_type == "SQL Server":
    st.info("💡 Para autenticación de Windows, deja los campos vacíos. Se usará la configuración por defecto.")
    conn_args['server'] = st.text_input("Nombre del Servidor (opcional):", 
                                       help="Ej: localhost, servidor\\SQLEXPRESS. Vacío = detección automática")
    conn_args['database'] = st.text_input("Nombre de la Base de Datos (opcional):", 
                                         help="Vacío = master por defecto")
elif db_type in ["PostgreSQL", "MySQL"]:
    conn_args['host'] = st.text_input("Host:")
    conn_args['port'] = st.text_input("Puerto:")
    conn_args['user'] = st.text_input("Usuario:")
    conn_args['password'] = st.text_input("Contraseña:", type="password")
    conn_args['database'] = st.text_input("Nombre de la Base de Datos:")
elif db_type == "SQLite":
    conn_args['database_path'] = st.text_input("Ruta al archivo de la Base de Datos (.db):")

input_usuario = st.text_input("Introduce tu consulta en lenguaje natural:")
        ...
        try:
            with st.spinner(f"Conectando a {db_type} y procesando tu consulta..."):
                ...
                if db_type == "SQL Server":
                    ...
                    db_chain, engine = get_db_chain(db_type, conn_args_filtered if conn_args_filtered else None)
                else:
                    db_chain, engine = get_db_chain(db_type, conn_args)
                respuesta = consulta(db_chain, engine, input_usuario, db_type)
...
if st.session_state.query_results is not None:
    st.divider()
    st.subheader(f"📋 Última consulta: {st.session_state.last_query}")
    ...
    if isinstance(st.session_state.query_results, pd.DataFrame):
        ...
        if st.session_state.query_dataframe is not None:
            st.subheader("📊 Datos extraídos para visualización")
            st.dataframe(st.session_state.query_dataframe)
...
if (st.session_state.query_dataframe is not None and 
    ...
    with st.expander("📊 Resumen Estadístico", expanded=False):
    ...
    # Gráficos personalizados (optimizado)
    with st.expander("🎨 Crear gráfico personalizado"):
        chart_types = ["Barras", "Torta", "Dispersión", "Histograma", "Líneas"]
        selected_chart = st.selectbox("Tipo de gráfico:", chart_types)
...
if st.session_state.ai_insights is not None and st.session_state.ai_insights.strip():
    ...
    <div style="background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); 
        ...
        <h3 style="margin: 0; display: flex; align-items: center;">
            🤖 Asistente Inteligente - Análisis y Recomendaciones
        </h3>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.container():
        ...
        # Mostrar insights en una caja estilizada
    ...
    if st.session_state.follow_up_queries:
        ...
        # Mostrar botones de consultas sugeridas
        for i, suggested_query in enumerate(st.session_state.follow_up_queries):
            col1, col2 = st.columns([4, 1])
            with col1:
                st.markdown(f"**{i+1}.** {suggested_query}")
            with col2:
                if st.button("📋", key=f"copy_{i}", help="Copiar consulta"):
                    # Simular copiar al portapapeles (en una app real usarías JS)
                    st.success("¡Copiado!")
        
        # Opción para ejecutar consulta sugerida directamente
        st.markdown("---")
        selected_query = st.selectbox(
            "💫 Ejecutar consulta sugerida directamente:", 
            ["Seleccionar..."] + st.session_state.follow_up_queries,
            key="suggested_query_select"
        )
        
        if selected_query != "Seleccionar..." and st.button("🚀 Ejecutar Consulta Sugerida"):
            # Preparar para ejecutar la consulta sugerida
            st.session_state.suggested_query_to_run = selected_query
            st.info(f"💡 Consulta seleccionada: {selected_query}")
            st.info("👆 Copia esta consulta en el campo de arriba y presiona 'Consultar'")
